...
def draw_contour(contour_list, size=[260,260]):
    background = np.zeros(size)
    for i, contour in enumerate(contour_list):
        background[contour[:, 0], contour[:, 1]] = i+1
...
def createPolyDataNormalsFromArray(img_array, spacing=[1.0, 1.0, 1.0], origin=[0.0, 0.0, 0.0], use_flying_edges=False, get_largest_connect_region=True):
    """
...
def slicing_xy_vtk(aligned_spine_polydata, z_coordinate):
    ...
    if points1.shape[0] < 45 or points0.shape[0] < 45:
...
def PCA(data, sort=True):
    ...
    return eigenvalues, eigenvectors.T, X_mean[:,0]
...
def save_coordinate(case_name, origin, coordinates, plane_center_point_L=None, plane_center_point_R=None,\
    ...
    save_path = os.path.join('../DeepSpineData/test_data', case_name+'.txt')
...
def getReferenceSpineAxisZNormal(cur_spine_stl_file):
    ...
    next_spine_stl_file = short_name + "%02d.stl"%(cur_label_index + 1)
...
def createCutPolyData(source_poly_data, cut_plane1, cut_plane2):
    """
    :param source_poly_data:
    :param cut_planes:
    :return:
    """

    clip_poly_data1 = vtkClipPolyData()
    clip_poly_data1.SetInputData(source_poly_data)
    clip_poly_data1.SetClipFunction(cut_plane1)
    clip_poly_data1.GenerateClippedOutputOn()
    clip_poly_data1.Update()

    clip_poly_data2 = vtkClipPolyData()
    clip_poly_data2.SetInputData(clip_poly_data1.GetOutput())
    clip_poly_data2.SetClipFunction(cut_plane2)
    clip_poly_data2.GenerateClippedOutputOn()
    clip_poly_data2.Update()

    return clip_poly_data2.GetOutput()
